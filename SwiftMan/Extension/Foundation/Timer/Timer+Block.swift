...
public extension Timer {
    ...
    public class func m_timerOnCurrentCommonModes(timeInterval: TimeInterval, block: ()->(),  repeats: Bool) -> Timer{
        let timer = Timer(timeInterval: timeInterval, target: self, selector: #selector(Timer.executeBlockWithTimer(_:)), userInfo: unsafeBitCast(block, to: AnyObject.self), repeats: repeats)
